...
function calculateKPIs(ad_spend: number, leads: number, conversions: number) {
    ...
    if (leads > 0) {
        cost_per_lead = Math.round((ad_spend / leads) * 100) / 100; // Round to 2 decimals
    }

    // Conversion Rate = (conversions / leads) * 100
    if (leads > 0) {
        conversion_rate = Math.round((conversions / leads) * 100 * 100) / 100; // Round to 2 decimals
    }
 ...
 * Query params: ?campaign_id=xxx&client_id=xxx
 * 
...
export async function GET(request: NextRequest): Promise<NextResponse<GetMetricsResponse>> {
    try {
        ...
        // Step 4: For account managers, filter by assigned clients
